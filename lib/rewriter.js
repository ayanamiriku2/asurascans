...
function rewriteHtml(html, origin, mirror, proto) {
  ...
  // -----------------------------------------------------------------------
  $('#google-btn').remove();
  $('.google-btn').closest('div.relative').remove();
  $('a[href*="/api/auth/google"]').closest('div.relative').remove();
  $('#google-last-used').remove();
  $('.or-divider').remove();
  ...
  // -----------------------------------------------------------------------
  // 12. Inject fetch/XHR interceptor to rewrite api.mirror → mirror
  //     Handles dynamically constructed API URLs at runtime
  // -----------------------------------------------------------------------
  const apiInterceptScript = `
<script>
(function(){
  var apiHost = "api.${mirror}";
  var mirrorHost = "${mirror}";
  var proto = "${proto}";
  function fixUrl(url){
    if(typeof url==="string"){
      return url
        .replace(new RegExp("https?://"+apiHost.replace(/\\./g,"\\\\."),"gi"), proto+"://"+mirrorHost)
        .replace(new RegExp("//"+apiHost.replace(/\\./g,"\\\\."),"gi"), "//"+mirrorHost);
    }
    if(url instanceof URL){
      if(url.hostname===apiHost){url.hostname=mirrorHost;url.protocol=proto+":";}
    }
    if(url instanceof Request){
      return new Request(fixUrl(url.url),url);
    }
    return url;
  }
  var origFetch=window.fetch;
  window.fetch=function(input,init){
    return origFetch.call(this,fixUrl(input),init);
  };
...
function replaceOrigin(str, origin, mirror, proto) {
  ...
  // Replace api.origin FIRST (before general origin replacement)
  // so api.asurascans.com → mirror (not api.mirror)
  str = str
    .replace(new RegExp(`https://api\\.${esc(origin)}`, "gi"), `${proto}://${mirror}`)
    .replace(new RegExp(`http://api\\.${esc(origin)}`, "gi"), `${proto}://${mirror}`)
    .replace(new RegExp(`//api\\.${esc(origin)}`, "gi"), `//${mirror}`);
  return str
