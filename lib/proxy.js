...
async function proxyAndRewrite(req, res, { origin, mirror, proto }) {
  ...
  // Fix referer & origin headers so the API accepts the request
  if (upHeaders["referer"]) {
    upHeaders["referer"] = upHeaders["referer"]
      .replace(new RegExp(escapeRegex(mirror), "gi"), origin);
  }
  if (upHeaders["origin"]) {
    upHeaders["origin"] = upHeaders["origin"]
      .replace(new RegExp(escapeRegex(mirror), "gi"), origin);
  }
  ...
  const upstreamRes = await fetch(upstreamUrl.toString(), fetchOpts);
  ...
  if (isHtml) {
    ...
    // Replace api.origin FIRST so api.asurascans.com → mirror/api (not api.mirror)
    text = text
      .replace(new RegExp(`https://api\\.${escapeRegex(origin)}`, "gi"), `${proto}://${mirror}`)
      .replace(new RegExp(`http://api\\.${escapeRegex(origin)}`, "gi"), `${proto}://${mirror}`)
      .replace(new RegExp(`//api\\.${escapeRegex(origin)}`, "gi"), `//${mirror}`)
      .replace(new RegExp(`api\\.${escapeRegex(origin)}`, "gi"), mirror);
    // Then replace main origin
    text = text.replace(
      new RegExp(escapeRegex(origin), "gi"),
      mirror
    );
